...
class App(QMainWindow):
    def __init__(self):
        ...
        self.project_name = None  # Имя проекта, полученное от сервера в случае доступности

        self.initUI()           # 1. Настройка основного окна
        self.runServerDialog()  # 2. Создание диалогового окна для выбора сервера

    def runServerDialog(self):
        ...
        if result == QDialog.DialogCode.Accepted:
            ...
            self.infoLabel.setText(project_name)
        else:
            print("⛔️ Отменено")
    ...
    def initUI(self):
        ...
        self.setGeometry(100, 100, 640, 820)  # Размеры окна 
        ...
        btnServers = QPushButton("SERVER", self) # self указывает, что эта кнопка будет дочерним элементом текущего окна или виджета, в котором она создается
        btnServers.clicked.connect(self.runServerDialog)  # Запуск диалогового окна выбора сервера
        btnServers.setStyleSheet(btnStyle)       # Оформление кнопки
        ...
        buttonLayout.addWidget(btnServers)      # Добавление кнопки SERVER
    ...
    def request_project_name(self):
        ...
        try:
            response = requests.get(API_URL_INFO, timeout=3)  # Отправка запроса 
            ...
            msg = QMessageBox()
            msg.setIcon(QMessageBox.Icon.Warning)
            msg.setText("⚠️ Timeout. The request timed out.")
            msg.setWindowTitle("Ошибка")
            msg.exec()
            return None
            ...
            msg = QMessageBox()
            msg.setIcon(QMessageBox.Icon.Warning)
            msg.setText(f"⚠️ An error occurred: {e}")
            msg.setWindowTitle("Ошибка")
            msg.exec()
            return None
